document.addEventListener('DOMContentLoaded', () => {
        ...
        setTimeout(() => {
            try {
                ...
                if (encrypted) {
                    ...
                    localStorage.setItem(`${userPrefix}_encrypted-result`, encrypted);
        ...
        setTimeout(() => {
            try {
                ...
                if (decrypted) {
                    ...
                    localStorage.setItem(`${userPrefix}_decrypted-result`, decrypted);
    ...
    stateElements.forEach(id => {
        ...
        if (el) {
            el.addEventListener('input', () => {
                localStorage.setItem(`${userPrefix}_${id}`, el.value);
            });
        }
    });
    ...
    function loadStoredKeys() {
        ...
        stateElements.forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                const storedValue = localStorage.getItem(`${userPrefix}_${id}`);
                if (storedValue) {
        ...
        // Enable copy button for encrypted result if it has value
        if (encryptedResultEl && encryptedResultEl.value && copyEncryptedBtn) {
